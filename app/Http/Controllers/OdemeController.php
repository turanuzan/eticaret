...
class OdemeController extends Controller
{
    ...
    {
        if(!auth()->check()){ // giriş işlemi yapıldı mı?
            return redirect()->route('kullanici.oturumac')
                ->with('mesaj_tur','info')
                ->with('mesaj','Ödeme işlemi için oturum açmanız veya kullanıcı kaydı oluşturmanız gerekmetedir.');
        }
    ...
    public function odemeyap()
    {
        ...
        return redirect()->route('siparisler')
            ...
            ->with('mesaj','Ödemeniz başarıyla alınmıştır.');
    }
}
